...
static button_t buttonElement2={
    .buttonGpioNum=BUTTON_IN3,
    .buttonPressedStateLogicLevel=0,
    .eventMillis=0,
    .pressCount=0,
    .pressDetectionState=0
...
uint8_t inputSwitchStates(int buttonIndex){
        ...
        case 1:
            if(gpio_get_level(buttonElemetArray[buttonIndex].buttonGpioNum)==buttonElemetArray[buttonIndex].buttonPressedStateLogicLevel){
                ...
                if(millisDiff>MINIMUM_BUTTON_PRESS_PERIOD){
                    ...
                    if(buttonIndex == 2){
                        button_details.intensity += 1;
                        button_details.intensity %= 2;
                        xQueueSend(app_buttonDetailsQueue, &button_details, 10); 
                    }
                    else{
                        if(button_details.intensity < 10 && buttonIndex == 0)
                            button_details.intensity += 2;
                        else if(button_details.intensity > 0 && buttonIndex == 1)
                            button_details.intensity -= 2;
                        xQueueSend(device_FanRegulateQueue, &button_details.intensity, 10); 
                    }
                }
            }
            else{
...
void app_userInputInit()
{
    ...
    buttonElemetArray[2]=buttonElement2;  //Fan ON/OFF
