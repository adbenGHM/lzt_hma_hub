                          ...
                          int32_t event_id, void *event_data)
{
    ...
    {
        if (s_retry_num < APP_CONFIG_ESP_MAXIMUM_WIFI_RETRY)
        {
            esp_wifi_connect();
            s_retry_num++;
...
app_status_t app_wifiStaInit(void)
{
    ...
    app_status_t returnStatus=APP_STATUS_ERROR;
    ...
    if (bits & WIFI_CONNECTED_BIT)
    {
        ESP_LOGI(TAG, "connected to ap SSID: %s password:%s",
                 appConfig.wifiSsid, appConfig.wifiPassword);
    ...
    else if (bits & WIFI_FAIL_BIT)
    {
        ESP_LOGI(TAG, "Failed to connect to SSID: %s, password:%s",
                 appConfig.wifiSsid, appConfig.wifiPassword);
        returnStatus= APP_STATUS_ERROR_STA_MODE_CONNECTION_FAILED;
    }
    else
    {
        ESP_LOGE(TAG, "UNEXPECTED EVENT");
        returnStatus= APP_STATUS_ERROR;
    }
