...
app_status_t app_saveConfig()
{
    ...
    required_size = sizeof(storeBlock);
    ...
    err = nvs_set_blob(nvsHandle, APP_CONFIG_NVS_NAMESPACE, (char *)&storeBlock, required_size);
    if (err != ESP_OK)
    {
...
app_status_t app_loadConfig()
{
    ...
    if (required_size > 0)
    {
        memset(&storeBlock,'\0',sizeof(storeBlock));
        err = nvs_get_blob(nvsHandle, APP_CONFIG_NVS_NAMESPACE, (char *)&storeBlock, &required_size);
        if (err != ESP_OK)
        {
