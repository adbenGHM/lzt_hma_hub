...
void isrTakeInputTask(void* pvParameters){
    while(1)
    {
        ...
        for(int buttonIndex=0;buttonIndex<NUM_OF_BUTTON_INPUTS;buttonIndex++){
            if(gpio_get_level(buttonElemetArray[buttonIndex].buttonGpioNum)==0){
                buttonElemetArray[buttonIndex].eventMillis=getMilis();
                if(buttonIndex==0){
                    if(buttonElemetArray[buttonIndex].pressCount<10)
                        buttonElemetArray[buttonIndex].pressCount +=2;
                }
                else{
                    if(buttonElemetArray[buttonIndex].pressCount>0)
                            buttonElemetArray[buttonIndex].pressCount -=2;
                }
                xQueueSend( device_FanRegulateQueue, &buttonElemetArray[buttonIndex].pressCount, 10);
            }
...
void input_ledIndicator(uint8_t level){
